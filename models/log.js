...
if(objConfig.appenders){
	...
	for(var i=0;i<objConfig.appenders.length;i++){
		...
		if(baseDir!=null){
			...
			else item["filename"]=baseDir+item["filename"];
		}
		var fileName=item["filename"];
		if(fileName==null)continue;
		var pattern=item["pattern"];
		if(pattern!=null)fileName+=pattern;
...
log.writeDebug=function(msg){
	if(msg==null)msg="";
	logDebug.debug(msg);
};
log.writeInfo=function(msg){
	if(msg==null)msg="";
	logInfo.info(msg);
};
log.writeWarn=function(msg){
	if(msg==null)msg="";
	logWarn.warn(msg);
};
log.writeErr=function(msg,exp){
	if(msg==null)msg="";
	if(exp!=null)msg+="\r\n"+exp;
	logErr.error(msg);
};
//配合express用的方法  
log.use = function(app){  
